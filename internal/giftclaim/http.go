...
<meta name="color-scheme" content="light dark"><title>Claim · {{.}}</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<script defer src="https://unpkg.com/@tonconnect/ui@2.3.1/dist/tonconnect-ui.min.js"></script>
<style>:root{font-family:Arial,system-ui,sans-serif;color:#17212b;background:#f4f6f8;--tg-accent:#2481cc;--tg-muted:#8794a1;--tg-card:#fff;--tg-line:#d8e0e8}*{box-sizing:border-box}body{margin:0;min-height:100vh;padding:calc(18px + env(safe-area-inset-top)) 16px;background:linear-gradient(#e9f3fc,#f4f6f8 45%)}main{max-width:520px;margin:auto}.brand{font-weight:700;color:#52606d;margin:4px 4px 14px}.card{background:var(--tg-card);border:1px solid var(--tg-line);border-radius:12px;padding:22px;box-shadow:0 2px 12px #17212b12}.gift{text-align:center;font-size:50px;margin-bottom:8px}.eyebrow{text-align:center;color:var(--tg-accent);font-size:12px;font-weight:700;letter-spacing:.09em;text-transform:uppercase}h1{text-align:center;margin:7px 0;font-size:28px}p.lead{text-align:center;color:var(--tg-muted);line-height:1.5;margin:0 0 20px}label{display:block;color:#52606d;font-size:13px;font-weight:700;margin:14px 0 7px}input,select{width:100%;border:1px solid #c9d3dd;background:var(--tg-card);color:inherit;padding:12px;border-radius:8px;font:inherit;outline:none}input:focus,select:focus{border-color:var(--tg-accent);outline:2px solid #2481cc33}button,.wallet-link{width:100%;border:0;border-radius:8px;padding:13px;margin-top:14px;background:var(--tg-accent);color:#fff;font:inherit;font-weight:700;cursor:pointer;text-align:center;text-decoration:none;display:block}button:disabled,select:disabled{opacity:.45}.wallet-link[hidden]{display:none}.status{min-height:22px;text-align:center;margin-top:14px;color:var(--tg-muted);font-size:13px;overflow-wrap:anywhere}.status.error{color:#c83d42}.status.done{color:#168552}.facts{display:none;gap:9px;margin-top:17px}.facts.show{display:grid}.fact{background:#f4f7fa;border:1px solid var(--tg-line);border-radius:8px;padding:12px;overflow-wrap:anywhere}.fact span{display:block;color:var(--tg-muted);font-size:10px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px}.fact strong{font-size:12px}.note{color:var(--tg-muted);font-size:11px;text-align:center;margin-top:18px}@media(prefers-color-scheme:dark){:root{color:#e8edf2;background:#17212b;--tg-card:#212a33;--tg-line:#35424f;--tg-muted:#aab7c4}body{background:linear-gradient(#1b2d3d,#17212b 50%)}label{color:#c5d0da}.fact{background:#26333d}}</style></head>
